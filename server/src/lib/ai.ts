...
function formatPlanResponse(
  ...
  const plan: Omit<TrainingPlan, "id" | "userId" | "version" | "createdAt"> = {
    overview: {
      ...
      frequency:
        aiResponse.overview?.frequency || `${profile.days} days per week`,
...
function buildPrompt(profile: UserProfile): string {
  ...
  return `Create a personalized ${profile.days}-day per week training plan for someone with the following profile:
  
Goal: ${goalMap[profile.goal] || profile.goal}
Experience Level: ${experienceMap[profile.experience] || profile.experience}
Session Length: ${profile.session} minutes per session
...
Generate a complete training plan in JSON format with this exact structure:
{
  ...
  - Each workout should fit within ${profile.session} minutes
