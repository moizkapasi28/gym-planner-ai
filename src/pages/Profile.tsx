...
export default function Profile() {
  ...
  if (auth.isLoading || auth.isPlanLoading) {
    return (
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[var(--color-muted)]">
              Version {plan.version} • Created {formatDate(plan.created_at)}
            </p>
          </div>
          ...
          <Button
            ...
            disabled={auth.isGenerating}
          >
            <RefreshCcw
              className={` w-4 h-4 ${auth.isGenerating ? "animate-spin" : ""}`}
            />
            Regenerate Plan
          </Button>
        </div>
